...
/**
 * EveryGSM 프로젝트를 datagsm에 등록/수정하는 공용 로직. AdminApproveProjectService(승인 시 단건
 * 반영)와 AdminSyncRepoAndTechStackToDatagsmService(이미 등록된 프로젝트 일괄 재동기화)가 함께 사용한다.
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class DatagsmProjectSyncSupport {

    private final ProjectRepository projectRepository;
    private final DatagsmApiClient datagsmApiClient;
    private final DataGsmOpenApiClient dataGsmOpenApiClient;

    public void registerToDatagsm(ProjectJpaEntity project) {
    ...
    public void updateInDatagsm(ProjectJpaEntity project) {
        Long clubId = resolveClubId(project.getAffiliation());
        List<Long> participantIds = resolveParticipantIds(project);
